...
vec4 scanlineWeights(float distance, vec4 color)
{
	...
	vec4 weights = vec4(distance / 0.35);
...
function screen:set( angle, scale )
	...
	if self.angle == math.rad( 90 ) then
		w, h = self.w * self.scale, self.h * self.scale
		self.ox, self.oy = 0, 320
	else
		h, w = self.w * self.scale, self.h * self.scale
		self.ox, self.oy = 0, 0
	end
...
function screen:zoom( value )
	...
	local s = self.scale + value
	if s < 1 then
		s = 1
	elseif s * self.h > limit then
		s = s - 1
	end
	self:set( nil, s )
end
